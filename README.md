...
- 真人先手：约 2 秒 MCTS，剩余不超过 12 格时精确搜索；这一模式明确不声称已有后手不败证明。
- 本地双人：同一设备轮流落子；AI 模式可撤回到上一真人决策点，双人模式可撤销上一手。
- 理论结果曲线：显示双方从当前局面开始完美应对时的结果，精确胜／和／负固定为 100／50／0；前期近似估值使用虚线，且明确提醒后续失误仍会改变实际结局。
- 精确决策指标：进入剩余 12 格的精确阶段后，显示当前玩家的不败着法率和胜／和／负分支数；AI 先手模式改为显示 AI 在完美防守下保证的计分差。
- 招法提示：每个决策点在独立 Worker 中后台预运算，点击“查看提示”才揭示并高亮；开局使用 MCTS 强力估计，精确阶段先保证最佳胜／和／负等级，再选择让对手最容易偏离最佳防守的分支。
