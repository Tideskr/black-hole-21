...
import {
  ...
  scoreBoard, undoKeepCount, type Board, type Player, type RuntimeCertificate, type ScoreResult,
} from '../lib/game';
...
function trendFromMoves(moves: readonly MoveRecord[], mode: Mode): TrendPoint[] {
  ...
  for (const move of moves) {
    points.push({
      label: `${move.side === 'ai' ? 'AI' : move.side === 'human' ? '你' : move.side === 'player1' ? '玩家一' : '玩家二'} ${move.number}`,
...
function AdvantageChart({ points, mode }: { points: TrendPoint[]; mode: Mode }) {
  const width = 320;
  const height = 104;
  ...
  const certainty = current.certainty === 'proof' ? '策略保证' : current.certainty === 'exact' ? '精确搜索' : '局面估值';
  ...
  return (
    <section className="advantage-card" aria-label={`${label} ${current.value}`}>
      ...
      <div className="chart-caption"><span>{certainty}</span><span>{current.label}</span></div>
      <p>不是统计胜率：精确结果先落入胜／和／负区间，再按终局分差表示取胜难度；未搜索局面使用潜在黑洞的邻和差估值。</p>
    </section>
  );
}
    ...
    <section className="game-panel" aria-label="21 黑洞游戏">
      ...
      <div className="game-stage">
        ...
        <aside className="game-rail">
          <AdvantageChart points={trend} mode={mode} />
